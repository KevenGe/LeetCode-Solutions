...
def main() -> None:
    ...
    mdbook_dir_path = Path("./mdbook")
    for md_file_path in (mdbook_dir_path / "src-template").iterdir():
        ...
        with open(mdbook_dir_path / "src" / md_file_path.name, "w") as f:
            f.write(buf.getvalue())

    # generate md

    for ps in problemset:
        (mdbook_dir_path / "src" / "problems" / (str(ps["ID"]) + ". " + ps["TITLE_CN"] + ".md")).touch()
        
        buf = StringIO()
        my_template = Template(filename=str(mdbook_dir_path / "src-template" / "problems" / "problem.md"))
        ctx = Context(
            buf,
            x = ps
        )
        ...
        my_template.render_context(ctx)
        with open((mdbook_dir_path / "src" / "problems" / (str(ps["ID"]) + ". " + ps["TITLE_CN"] + ".md")), "w") as f:
            f.write(buf.getvalue())
